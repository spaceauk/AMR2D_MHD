...
void celledges(meshblock &dom,int nb) {
		...
		for (int j=0; j<dom.ny; j++) {
			for (int k=0; k<dom.nvar; k++) {
				...
				if (isnan(dom.dwdx[i][j][k]) or isnan(dom.dwdy[i][j][k])) {
					cout<<"YH: Nan values encountered - dx="<<dom.dwdx[i][j][k]<<", dy="<<dom.dwdy[i][j][k]<<
		...
		for (int j=0; j<dom.ny; j++) {
			for (int k=0; k<dom.nvar; k++) {
				if (CT_mtd and k==5) { 
					...
					dom.wxL[i][j][k]=dom.Bi[i+1][j][0][nb];
					dom.wxR[i][j][k]=dom.Bi[i][j][0][nb];
					continue;
				}
	...
	for (int i=0; i<dom.nx; i++) { // For y
		for (int j=0; j<dom.ny-1; j++) {
			for (int k=0; k<dom.nvar; k++) {
				if (CT_mtd and k==6) {
                                        dom.wyL[i][j][k]=dom.Bi[i][j+1][1][nb];
                                        dom.wyR[i][j][k]=dom.Bi[i][j][1][nb];
                                        continue;
                                }
				if (dom.limiter=="Cada3rd") {
