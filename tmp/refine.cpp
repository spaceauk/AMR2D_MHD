...
void refine(meshblock &dom, int nb, 
	    int son1, int son2, int son3, int son4) {
	// Update U
	for (int i=0;i<dom.nx;i++) {
	  for (int j=0;j<dom.ny;j++) {
	    for (int k=0;k<dom.nvar;k++) {
	      dom.U[i][j][k][son1]=dom.U[(i+nghosts)/2][(j+nghosts)/2][k][nb];
	      dom.U[i][j][k][son2]=dom.U[(i-nghosts+2)/2+dom.nx2][(j+nghosts)/2][k][nb];
	      dom.U[i][j][k][son3]=dom.U[(i+nghosts)/2][(j-nghosts+2)/2+dom.ny2][k][nb];
	      dom.U[i][j][k][son4]=dom.U[(i-nghosts+2)/2+dom.nx2][(j-nghosts+2)/2+dom.ny2][k][nb];
	    }		
	  }
	}
	...
	// YH: Note that icoord is written such that it is the x/y position
	...
	// Update icoords
	cout<<"YH: Refining giving sons=["<<son1<<","<<son2<<","<<son3<<","<<son4<<"]"<<endl;
	dom.U2W("refine",son1);
