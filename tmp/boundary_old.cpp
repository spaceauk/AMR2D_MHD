	  ...
	  switch(dom.innerbounds[nb][2]) {
		...
		//----------------------------------------------------LEFT-----------------------------------------------------
		  ...
		  case 2: // Left @ higher resolution 1
			  ...
			  n2=n1+2;
			  // Own's left
			  for (int j=1;j<=dom.ny2+nghosts/2;j++) {
			    int ny2j=dom.ny2+j-1;
			    for (int i=0;i<nghosts;i++) {
			      int i1=2*(i-1)-nghosts/2+dom.nxmax;
			      int i2=i1+1;
			      int j1=2*j-2; // YH: The use of -2 is only valid when NG=2. Must ensure that j1=j=nymin for general NG.
			      int j2=j1+1;
 			      for (int k=0;k<dom.nvar;k++) {
			  ...
			  for (int k=0;k<dom.nvar;k++) {
			    for (int i=0;i<nghosts;i++) {
			      Q[i][0][k][nbs]=Q[i][1][k][nbs];
			      Q[i][dom.ny-1][k][nbs]=Q[i][dom.ny-2][k][nbs];
			    }
			  }
			  // Neighbor's right
			  ...
			  for (int i=1;i<=nghosts;i++) {
			    for (int j=0;j<dom.ny;j++) {
			      for (int k=0;k<dom.nvar;k++) {
			        Q[i+dom.nxmax][j][k][n1]=Q[(i+3)/2][(j+2)/2][k][nbs];
			        Q[i+dom.nxmax][j][k][n2]=Q[(i+3)/2][(j)/2+dom.ny2][k][nbs];
		  	      }
			    }
			  }
			  break;
		  ...
		  case 5: // Right @ higher resolution 1
			  ...
			  for (int i=1;i<=nghosts;i++) {
				...
				// YH: Rmb to correct nxm1 to nghosts/2 for general b.c.s
			        Q[i-1][j][k][n1]=Q[(i+1)/2+dom.nxmax-nghosts/2][(j+2)/2][k][nbs];
			        Q[i-1][j][k][n2]=Q[(i+1)/2+dom.nxmax-nghosts/2][(j)/2+dom.ny2][k][nbs];
		  	      }
			    }
			  }
			  break;
		//----------------------------------------------------BOTTOM-----------------------------------------------------
		  ...
		  case 8: // Bottom @ higher resolution 1
			  ...
			  n2=n1+1;
			  // Own's bottom
			  for (int i=1;i<=dom.nx2+nghosts/2;i++) {
			    int nx2i=dom.nx2+i-1;
			    for (int j=0;j<nghosts;j++) {
			      int i1=2*i-2;
                              int i2=i1+1;
			      int j1=2*(j-1)-nghosts/2+dom.nymax;
			      int j2=j1+1;
 			      for (int k=0;k<dom.nvar;k++) {
			  ...
			  for (int k=0;k<dom.nvar;k++) {
			    for (int j=0;j<nghosts;j++) {
			      Q[0][j][k][nbs]=Q[1][j][k][nbs];
 			      Q[dom.nx-1][j][k][nbs]=Q[dom.nx-2][j][k][nbs];
			    }
			  }
			  // Neighbor's top
			  // to finer level (0th order interpolation)
			  for (int j=1;j<=nghosts;j++) {
			    for (int i=0;i<dom.nx;i++) {
			      for (int k=0;k<dom.nvar;k++) {
			        Q[i][j+dom.nymax][k][n1]=Q[(i+2)/2][(j+3)/2][k][nbs];
			        Q[i][j+dom.nymax][k][n2]=Q[(i)/2+dom.nx2][(j+3)/2][k][nbs];
		  	      }
			    }
			  }
			  break;
		//----------------------------------------------------TOP-----------------------------------------------------
		  ...
		  case 11: // TOP @ higher resolution 1
			  ...
			  for (int j=1;j<=nghosts;j++) {
			    for (int i=0;i<dom.nx;i++) {
			      for (int k=0;k<dom.nvar;k++) {
			        Q[i][j-1][k][n1]=Q[(i+2)/2][(j+1)/2+dom.nymax-nghosts/2][k][nbs];
			        Q[i][j-1][k][n2]=Q[(i)/2+dom.nx2][(j+1)/2+dom.nymax-nghosts/2][k][nbs];
		  	      }
			    }
			  }
			  break;
