...
void CT2D(meshblock &dom,real**** q,int nb,int step);
	...
	while (t<=dom.tEnd) {
		...
		for (int nb=0; nb<dom.lastActive; nb++) {
		    ...
		    for (int i = 0; i < dom.nx; i++) {
			  ...
			  if ((i>=dom.nxmin and i<=dom.nxmax) and (j>=dom.nymin and j<=dom.nymax)) {
				  res=(dom.ff[i+1][j][k]-dom.ff[i][j][k])/dom.dx[dom.lp[nb][0]]
					  +(dom.gg[i][j+1][k]-dom.gg[i][j][k])/dom.dy[dom.lp[nb][0]];
			  } else {res=0.;}
            		  dom.Us[i][j][k][nb] = dom.U[i][j][k][nb] - dt*res;
		        }
		      }
		    }
		    // Apply constrained transport
		    if (CT_mtd) {CT2D(dom,dom.Us,nb,1);}
		  }
		}  
		  ...
		  if (dom.leafs[nb]) {
			  ...
			  if ((i>=dom.nxmin and i<=dom.nxmax) and (j>=dom.nymin and j<=dom.nymax)) {
					  ...
					  +(dom.gg[i][j+1][k]-dom.gg[i][j][k])/dom.dy[dom.lp[nb][0]];
                          ...
                          dom.U[i][j][k][nb] = 0.5*(dom.U[i][j][k][nb] + dom.Us[i][j][k][nb] - dt*res);
                        }
                      }
                    }
		    // Apply constrained transport - if apply this, above k<dom.nvar-3 as don't need update mag field
                    if (CT_mtd) {CT2D(dom,dom.U,nb,2);}
