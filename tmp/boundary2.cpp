		  ...
		  case 5: // Right @ higher resolution 1
			  ...
			  for (int i=1;i<=nghosts;i++) {
				...
				// YH: Rmb to correct nxm1 to nghosts/2 for general b.c.s
			        dom.Us[i-1][j][k][n1]=dom.Us[(i+1)/2+dom.nxmax-nghosts/2][(j+2)/2][k][nbs];
			        dom.Us[i-1][j][k][n2]=dom.Us[(i+1)/2+dom.nxmax-nghosts/2][(j)/2+dom.ny2][k][nbs];
		  	      }
			    }
			  }
			  break;
		...
		//----------------------------------------------------TOP-----------------------------------------------------
		  ...
		  case 11: // TOP @ higher resolution 1
			  ...
			  for (int j=1;j<=nghosts;j++) {
			    for (int i=0;i<dom.nx;i++) {
			      for (int k=0;k<dom.nvar;k++) {
			        dom.Us[i][j-1][k][n1]=dom.Us[(i+2)/2][(j+1)/2+dom.nymax-nghosts/2][k][nbs];
			        dom.Us[i][j-1][k][n2]=dom.Us[(i)/2+dom.nx2][(j+1)/2+dom.nymax-nghosts/2][k][nbs];
		  	      }
			    }
			  }
			  break;
